...
const aliceChapter: ChapterData = {
  index: 0,
  ...
  // Simplified timing structure - each sentence gets a 2-second duration for demo
  // In production, this would come from forced alignment with actual audio
  timings: new TimingIndex({
    chapter: 0,
    sentences: Array.from({ length: 15 }, (_, i) => [i, i * 2, (i + 1) * 2] as [number, number, number]),
    words: Array.from({ length: 15 }, (_, i) => [
      [i, 0, i * 2, i * 2 + 1],
      [i, 1, i * 2 + 1, (i + 1) * 2],
    ]).flat() as [number, number, number, number][],
  }),
  // Librivox recording of Alice's Adventures in Wonderland (public domain)
